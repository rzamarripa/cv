angular
.module("casserole")
.controller("UsuariosEncargadosCtrl", UsuariosEncargadosCtrl);
function UsuariosEncargadosCtrl($scope, $meteor, $reactive,  $state, $stateParams, toastr) {
  ...
  this.nuevo = true;
	...
	this.subscribe('usuariosEncargados', ()=>{
		return [{}]
	});	 

  this.helpers({
	  usuariosEncargados : () => {
		  return Meteor.users.find({"profile.sucursal_id" : Meteor.user() != undefined ? Meteor.user().profile.sucursal_id : "", roles : ["encargado"]});
	  }
  });  
  ...
  this.nuevoUsuario = function()
  {
	...
	{		
      ...
      return;
		...
		console.log(usuario);
		Meteor.call('createUsuario', usuario, 'encargado', function(error, result){
			if(error){
				toastr.error('Error al guardar los datos.');
				console.log(error);
			}else{
	...
	this.actualizar = function(usuario,form)
	{
      ...
      return;
		...
		usuario.profile.nombreCompleto = usuario.profile.nombre  + " " + usuario.profile.apPaterno + " " + (usuario.profile.apMaterno ? usuario.profile.apMaterno : "");
		console.log(usuario);
		Meteor.call('updateUsuario', usuario, "encargado");
	...
	this.cambiarEstatus = function(id)
	{
		...
		if(usuario.profile.estatus == true)
			...
			usuario.profile.estatus = true;
		Meteor.call('modificarUsuario', usuario, "encargado");
  };
